...
entries = 0 
exits = 0
# Create 2 points that will act as our virtual object entering and exitings boundary
virtual_obj_x = [-0.05, 0.05]
virtual_obj_y = [0, 0]
sign = 1
...
def serialConfig(configFileName):
    ...
    Dataport = serial.Serial('/dev/tty.SLAB_USBtoUART1', 921600)
    ...
    if dataOk and len(detObj["x"]) > 0:
        ...
        if frameNumber % 5 == 0:
            ...
            if virtual_obj_y[0] <= 0:
                sign = 1
            virtual_obj_y[0], virtual_obj_y[1] = virtual_obj_y[0] + sign * 0.1, virtual_obj_y[1] + sign * 0.1
...
def check_entries_exits(xi, yi, x, y):
    ...
    if (x > BOUNDARY_X and x < (BOUNDARY_X + BOUNDARY_WIDTH)) and (y > BOUNDARY_Y and y < (BOUNDARY_Y + BOUNDARY_HEIGHT)):
        if (xi < BOUNDARY_X or xi > (BOUNDARY_X + BOUNDARY_WIDTH)) or (yi < BOUNDARY_Y or yi > (BOUNDARY_Y + BOUNDARY_HEIGHT)):
            entries += 1
